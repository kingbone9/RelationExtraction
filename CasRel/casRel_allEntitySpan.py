...
class Batch:
    ...
    def create_label(self, inner_triples, inner_input_ids, seq_len, entities):
        ...
        if not inner_triples:
            random_sub = choice(entities)
            sub_ids = self.tokenizer(random_sub['name'], add_special_tokens=False)['input_ids']
            subs_head_idx = self.find_head_idx(inner_input_ids, sub_ids)

            for sub_start in subs_head_idx:
                inner_sub_head[sub_start] = 1
                inner_sub_tail[sub_start + len(sub_ids) - 1] = 1
                inner_sub_head2tail[sub_start:sub_start + len(sub_ids)] = 1
        ...
        if s2ro_map:
            ...
            idx_diff = sub_tail_idx - sub_head_idx
            subs_start_idx = s2start_map[(sub_head_idx, sub_tail_idx)]
            for sub_start in subs_start_idx:
                inner_sub_head[sub_start] = 1
                inner_sub_tail[sub_start + idx_diff] = 1
                inner_sub_head2tail[sub_start:sub_start + idx_diff + 1] = 1
            inner_sub_len = torch.tensor([(idx_diff + 1) * len(subs_start_idx)], dtype=torch.float)
    ...
    for step, (text, triple, entity) in enumerate(train_iter):
        ...
        if step % 500 == 1:
            ...
            if triple_f1 > best_triple_f1:
                ...
                torch.save(model.state_dict(), 'best_f1_allES_newTrainFunc_noEE.pth')
                print('------------------best model--------------------')
            print(df)
